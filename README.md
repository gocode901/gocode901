<h1 align="center">Hey 👋, I'm Satish Saini</h1>
<h3 align="center">Full Stack Software Engineer | Building Scalable Systems</h3>
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=25&duration=3000&color=00F7FF&center=true&vCenter=true&width=600&lines=Full+Stack+Engineer;Building+Scalable+Systems;Clean+Code+%7C+Real+World+Impact;Always+Learning+%26+Improving" />
</p>
<p align="center">
  <a href="https://www.linkedin.com/in/satishsaini02/">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin" />
  </a>
  <a href="satishsaini02062004@gmail.com">
    <img src="https://img.shields.io/badge/Email-red?style=flat&logo=gmail" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=your-github&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

## 🚀 About Me

- 💻 Full Stack Engineer building **production-grade, scalable applications**
- 🏗️ Experienced in **multi-tenant systems, RBAC, and secure authentication**
- ⚡ Focused on **clean architecture, performance, and maintainability**
- 🧠 Exploring **system design, backend scaling & AI integrations**
- 🌍 Passionate about solving **real-world problems through tech**

---

## 💼 Key Projects

### 🧩 Safety Management System (SMS)
- Multi-tenant enterprise platform with **Keycloak SSO + RBAC**
- Built using **NestJS, Next.js, PostgreSQL, Prisma**
- Designed for **scalability (1,000+ orgs, millions of documents)**

### 📊 Excel Data Processing System
- Automated **data parsing, validation, and ingestion pipeline**
- Built with **TypeScript + xlsx + backend validation layers**

### 🔐 Full Stack Authentication System
- Secure auth system using **JWT, role-based access, API protection**
- Designed for **real-world production use cases**

---

## 🛠️ Tech Stack

### 💻 Languages
![C++](https://img.shields.io/badge/C++-blue?style=flat&logo=c%2B%2B)
![Python](https://img.shields.io/badge/Python-yellow?style=flat&logo=python)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=flat&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-blue?style=flat&logo=typescript)
![SQL](https://img.shields.io/badge/SQL-grey?style=flat&logo=postgresql)

### ⚙️ Frameworks & Tools
![React](https://img.shields.io/badge/React-black?style=flat&logo=react)
![Next.js](https://img.shields.io/badge/Next.js-black?style=flat&logo=next.js)
![Node.js](https://img.shields.io/badge/Node.js-green?style=flat&logo=node.js)
![NestJS](https://img.shields.io/badge/NestJS-red?style=flat&logo=nestjs)
![Prisma](https://img.shields.io/badge/Prisma-black?style=flat&logo=prisma)
![Zod](https://img.shields.io/badge/Zod-blue?style=flat)

### 🗄️ Databases & Infra
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?style=flat&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-green?style=flat&logo=mongodb)
![Docker](https://img.shields.io/badge/Docker-blue?style=flat&logo=docker)
![AWS](https://img.shields.io/badge/AWS-orange?style=flat&logo=amazonaws)
![Keycloak](https://img.shields.io/badge/Keycloak-blue?style=flat)

---

## 🧠 Core Strengths

- 🏗️ Scalable System Design  
- 🔐 Authentication & Authorization (JWT, RBAC, CASL)  
- ⚡ API Design & Performance Optimization  
- 🧩 Monorepo Architecture (Turborepo)  
- 🛠️ Production-Ready Development  

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=your-githu&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=your-githu&theme=tokyonight" />
</p>

---

## 🚀 Currently Exploring

- ⚡ Backend Scaling (Redis, Queues, Microservices)
- 🤖 AI/ML + Intelligent Systems
- 🧠 Advanced System Design

---

## 🎯 Mission

> Build systems that scale, solve real problems, and create long-term impact.

---

## 📫 Let's Connect

- 💼 LinkedIn:https://www.linkedin.com/in/satishsaini02  
- 📧 Email: satishsaini02062004@gmail.com  

---

⭐ *"I don’t just write code — I build systems that handle scale."*
